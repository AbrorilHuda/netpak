...
export function meta({}: Route.MetaArgs) {
  return [
    { title: 'Edit Pelanggan - Transaksi Kuota' },
  ];
}
...
export default function EditCustomer() {
  ...
  const loadCustomer = async () => {
    try {
      ...
      if (data) {
        setFormData({
          name: data.name === 'Tanpa Nama' ? '' : data.name,
          phone: data.phone || '',
                ...
                {!showConfirmDelete ? (
                  ...
                  <div className="p-4 border border-rose-100 bg-rose-50/30 rounded-2xl space-y-3 animate-fade-in-up">
                    <p className="text-xs font-bold text-rose-700 text-center">
                      Apakah Anda yakin ingin menghapus pelanggan ini? Tindakan ini tidak bisa dibatalkan.
                    </p>
